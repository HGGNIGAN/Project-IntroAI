...
class GreedyBestFirstSolver(NonogramSolver):
    ...
    def _solve_internal(self):
        ...
        while open_set and iterations < max_iterations:
            ...
            for val in possible_vals:
                ...
                new_state = self._apply_constraint_propagation(new_state)
    ...
    def _apply_constraint_propagation(self, state):
        ...
        while changed:
            ...
            for i in range(state.m):
                if not state.rows_possible[i]: return None
                cells = self._get_deterministic_cells(state.rows_possible[i])
                for j, val in cells:
                    if state.board[i][j] == 0:
                        state.board[i][j] = val
                        state.cols_possible[j] = self._remove_possibilities(
                            state.cols_possible[j], i, val
                        )
                        if not state.cols_possible[j]: return None
                        changed = True
            for j in range(state.n):
                if not state.cols_possible[j]: return None
                cells = self._get_deterministic_cells(state.cols_possible[j])
                for i, val in cells:
                    if state.board[i][j] == 0:
                        state.board[i][j] = val
                        state.rows_possible[i] = self._remove_possibilities(
                            state.rows_possible[i], j, val
                        )
                        if not state.rows_possible[i]: return None
                        changed = True
        return state
    ...
    def _get_next_cell(self, state):
        ...
        for i in range(state.m):
            for j in range(state.n):
                if state.board[i][j] == 0:
                    row_vals = set(p[j] for p in state.rows_possible[i])
                    col_vals = set(p[i] for p in state.cols_possible[j])
