        ...
        *\n
        ...
        class line():
            def __init__(self,
                ...
                """
                self.index: "which row/column is this in the puzzle?"
                """
                self.type = type
                self.length = length
                self.index = index
            ...
            def generatePossibilities(self):
                ...
                for _ in range(len(filledBlocks)):
                    emptyBlocks.append([EMPTY])
                    remainingBits -= 1
                emptyBlocks.append([]) # at the end
                emptyBlocks.insert(0, []) # at the beginning
                slots = len(emptyBlocks)

                # Consider the bits leftover after filling in the required EMPTYs
                result = []
                def emptyMBitsInKSlots(
                    ...
                    if k == 1: # base case
                        ...
                        for i in range(m+1):
                            ...
                            emptyMBitsInKSlots(m-1,k-1,current, result)
                            # backtrack
                            current.pop()
                emptyMBitsInKSlots(remainingBits, slots, [], result)

                # NOW we're actually creating blocks of EMPTYs or nothing
                merged_result = []
                for constructor in result:
                    merged = []
                    for required, optional in zip(emptyBlocks, constructor):
                        merged.append(required + optional)
                    merged_result.append(merged)

                # and finally, weave the EMPTY and FILLED blocks together
                for constructor in merged_result: # dude why am i writing like it's java lol
                    nested = []
                    for filled, empty in zip(filledBlocks, constructor[:-1:]):
                        nested.append(empty)
                        nested.append(filled)
                    nested.append(constructor[-1])
            ...
            def perpendicular(self):
                """
                Finds all perpendicular lines propagated to from the unchanging bits.
                """
                pass
        ...
        for (entryType, entryIndex) in greedyOrdering:
            processingQueue.put(processingEntry(R[entryIndex], None))
